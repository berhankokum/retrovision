...
const MovieDetail = () => {
  ...
  const commentsPerPage = 5; // Bir sayfada gösterilecek yorum sayısı
...
// kullanıcı bilgileri
        ...
        try {
          const response = await databases.listDocuments(
            ...
            [
              ...
              Query.limit(commentsPerPage), // Sayfa başına yorum sayısı
              Query.offset((currentPage - 1) * commentsPerPage), // Hangi sayfadan başlanacağını belirler
            ]
          );
  ...
  const handleDeleteComment = async (commentId) => {
    ...
    try {
      // Kullanıcının yetkisini kontrol et (örneğin, admin kontrolü veya yorum sahibi kontrolü)
      ...
      if (hasDeletePermission) {
        await databases.deleteDocument(
          ...
          "6786cd6f0015c093ae57", // Koleksiyon ID'si
          commentId
        );
  ...
  const handleRateMovie = async () => {
    ...
    try {
      ...
      const response = await databases.listDocuments(
        "6786af350005fff9f376", // Veritabanı ID'si
        "6786cf160024bc208fcb", // Koleksiyon ID'si
        [
          Query.equal("user_id", user.$id),
          Query.equal("movie_id", Number(id)),
        ]
      ...
      if (response.documents.length > 0) {
        ...
        return; // Yeni puanlama yapılmasını engelle
      }
  ...
  // Sayfa değiştirildiğinde çalışacak işlev
